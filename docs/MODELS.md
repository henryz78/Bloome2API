...
这份文档记录当前 Bloome2API 代理中已支持的模型，并把三层名字分开说明：

- **展示名**：前端 / 平台里看到的自然名称
- **公开调用名（alias）**：Bloome2API 对外要求用户填写的 `model`
- **实际上游模型**：Bloome 云端最终打到的 alias / modelVersion
...
- Gemini：非流式工具调用；`stream: true` 时由代理层伪流式输出 SSE，但 Bloome 上游仍是非流式，不要把 Gemini 当作真正流式模型使用
...
公开错误响应会保持安全默认值：返回稳定的 `error.type`、`error.code` 和 `request_id`，不暴露原始上游错误。只有 `BLOOME2API_DEV_MODE=true` 时才会额外返回 `error.detail`。
