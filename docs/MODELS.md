...
- Gemini：非流式工具调用；Bloome 上游当前只返回非流式，不要把 Gemini 当作真正流式模型使用
...
Gemini 走 Vertex 协议。网关在用户未传时默认补 `generationConfig.maxOutputTokens = 65536`，可用 `GEMINI_DEFAULT_MAX_TOKENS` 覆盖。
